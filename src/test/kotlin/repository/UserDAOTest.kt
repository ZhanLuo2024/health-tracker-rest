    ...
    inner class CreateUser {
        ...
        fun `multiple users added to table can be retrieved successfully`() {
            transaction {
                ...
                Assertions.assertEquals(user1, userDAO.findById(user1.id))
                Assertions.assertEquals(user2, userDAO.findById(user2.id))
                Assertions.assertEquals(user3, userDAO.findById(user3.id))
            }
        }
    }
    ...
    inner class ReadUsers {
        ...
        fun `get user by id that doesn't exist, results in no user returned`() {
            transaction {
                ...
                //Act & Assert
                Assertions.assertEquals(null, userDAO.findById(4))
            }
        }
        ...
        fun `get user by email that doesn't exist, results in no user returned`() {
            transaction {
                ...
                //Act & Assert
                Assertions.assertEquals(null, userDAO.findByEmail(nonExistingEmail))
            }
        }
        ...
        fun `get user by email that exists, results in correct user returned`() {
            transaction {
                ...
                //Act & Assert
                Assertions.assertEquals(user2, userDAO.findByEmail(user2.email))
            }
        }
    ...
    inner class UpdateUsers {
        ...
        fun `updating existing user in table results in successful update`() {
            transaction {
                ...
                Assertions.assertEquals(user3Updated, userDAO.findById(3))
            }
        }
